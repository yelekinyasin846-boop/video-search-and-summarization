...
def sanity_check(dataset_path: str, total_frames: int, output_log_file: str) -> None:
    """
    Perform a dataset sanity check on instance ID segmentation JSON files.

    This function verifies the integrity of instance ID segmentation mapping files by:
    ...
    for camera_dir in camera_dirs:
        ...
        frame_template = "instance_id_segmentation_mapping_{:05d}.json"
        missing_frames = []
        unreadable_frames = []
        empty_frames = []
        ...
        if missing_frames:
            print(f"  [MISSING] {len(missing_frames)} instance_id_segmentation_fast frames: {missing_frames[:5]}{'...' if len(missing_frames) > 5 else ''}")
            log_lines.append(f"  [MISSING] {len(missing_frames)} instance_id_segmentation_fast frames: {missing_frames}\n")
        else:
            print(f"  All instance_id_segmentation_fast frames are present.")
            log_lines.append(f"  All instance_id_segmentation_fast frames are present.\n")
        
        if empty_frames:
            print(f"  [EMPTY] {len(empty_frames)} instance_id_segmentation_fast frames: {empty_frames[:5]}{'...' if len(empty_frames) > 5 else ''}")
            log_lines.append(f"  [EMPTY] {len(empty_frames)} instance_id_segmentation_fast frames: {empty_frames}\n")
        else:
            print(f"  All instance_id_segmentation_fast frames are not empty.")
            log_lines.append(f"  All instance_id_segmentation_fast frames are not empty.\n")

        if unreadable_frames:
            print(f"  [UNREADABLE] {len(unreadable_frames)} instance_id_segmentation_fast frames: {unreadable_frames[:5]}{'...' if len(unreadable_frames) > 5 else ''}")
            log_lines.append(f"  [UNREADABLE] {len(unreadable_frames)} instance_id_segmentation_fast frames: {unreadable_frames}\n")
        else:
            print(f"  All instance_id_segmentation_fast frames are readable.")
            log_lines.append(f"  All instance_id_segmentation_fast frames are readable.\n")
...
if __name__ == "__main__":
    """
    Command-line interface for the instance ID segmentation JSON dataset sanity check tool.

    Usage:
        python dataset_sanity_check_instance_id_json.py --base_dir /path/to/dataset --total_frames 1000 
            [--output_log log.txt]

    The script will:
    1. Check all instance ID segmentation JSON files in the dataset for:
       - Completeness
       - JSON readability
       - Empty data
    ...
    if args.output_log is None:
        args.output_log = os.path.join(args.base_dir, "sanity_instance_id_json_check_log.txt")
