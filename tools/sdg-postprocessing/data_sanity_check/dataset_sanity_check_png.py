...
def sanity_check(dataset_path: str, total_frames: int, output_log_file: str) -> None:
    """
    Perform a dataset sanity check on distance-to-image-plane PNG files.

    This function verifies the integrity of distance-to-image-plane PNG files by:
    ...
    for camera_dir in camera_dirs:
        rgb_folder = os.path.join(camera_dir, "distance_to_image_plane_png")
        
        if not os.path.exists(rgb_folder):
            print(f"[WARN] Missing 'distance_to_image_plane_png' folder in {camera_dir}. Skipping.")
            log_lines.append(f"[WARN] Missing 'distance_to_image_plane_png' folder in {camera_dir}. Skipping.\n")
            continue
        ...
        frame_template = "distance_to_image_plane_{:05d}.png"
        missing_frames = []
        unreadable_frames = []
...
if __name__ == "__main__":
    """
    Command-line interface for the distance-to-image-plane PNG dataset sanity check tool.

    Usage:
        python dataset_sanity_check_png.py --base_dir /path/to/dataset --total_frames 1000 
            [--output_log log.txt]

    The script will:
    1. Check all distance-to-image-plane PNG files in the dataset for:
       - Completeness
       - Image readability
    ...
    if args.output_log is None:
        args.output_log = os.path.join(args.base_dir, "sanity_png_depth_map_check_log.txt")
