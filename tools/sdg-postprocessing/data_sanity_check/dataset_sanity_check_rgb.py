...
def sanity_check(dataset_path: str, total_frames: int, output_log_file: str) -> None:
    """
    Perform a dataset sanity check on RGB image sequences.

    This function verifies the integrity of RGB image sequences in a dataset by:
    - Checking for missing frames in the sequence
    - Verifying that each frame is readable
    ...
    :return: None
    :raises: No explicit exceptions, but logs errors for missing/unreadable frames
    ...
    for camera_dir in camera_dirs:
        rgb_folder = os.path.join(camera_dir, "rgb")
        
        if not os.path.exists(rgb_folder):
            print(f"[WARN] Missing 'rgb' folder in {camera_dir}. Skipping.")
        ...
        if missing_frames:
            print(f"  [MISSING] {len(missing_frames)} rgb frames: {missing_frames}")
            log_lines.append(f"  [MISSING] {len(missing_frames)} rgb frames: {missing_frames}\n")
        else:
            print(f"  All frames are present.")
            log_lines.append(f"  All frames are present.\n")

        if unreadable_frames:
            print(f"  [UNREADABLE] {len(unreadable_frames)} rgb frames: {unreadable_frames}")
            log_lines.append(f"  [UNREADABLE] {len(unreadable_frames)} rgb frames: {unreadable_frames}\n")
        else:
            print(f"  All rgb frames are readable.")
            log_lines.append(f"  All rgb frames are readable.\n")
...
if __name__ == "__main__":
    """
    Command-line interface for the RGB dataset sanity check tool.

    Usage:
        python dataset_sanity_check_rgb.py --base_dir /path/to/dataset --total_frames 1000 [--output_log log.txt]

    The script will:
    1. Check all RGB frames in the dataset for completeness and readability
    ...
    if args.output_log is None:
        args.output_log = os.path.join(args.base_dir, "sanity_rgb_check_log.txt")
